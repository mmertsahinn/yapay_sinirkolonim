...
def scrape_match_data(match: Dict):
    """Tek bir maç için hype verisini çeker."""
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="JSON Hype Spesifik Doldurucu")
    parser.add_argument("--threads", type=int, default=25, help="Thread sayısını belirtin (varsayılan: 25)")
    parser.add_argument("--log_interval", type=int, default=500, help="Yazdırma sıklığını maç sayısı cinsinden belirtin (varsayılan: 500 maç)")
...
def main():
    ...
    print("=" * 60)
    print("🚀 JSON HYPE SPESIFIK DOLDURUCU (MAKSİMUM HIZ MODU)")
    ...
    while True:
        ...
        # İşleme başla
        processed_count = 0
        batch_size = max(max_workers, 10)  # Batch size optimize edildi
        ...
        for i in range(0, len(todo_list), batch_size):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_idx):
                    ...
                    if result:
                        ...
                        # Her 25 maçta bir geri dönüt ver
            ...
            time.sleep(0.5)  # Thread'ler arasında kısa mola
